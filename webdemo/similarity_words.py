...
def getvector(namemethod):
    ...
    if namemethod == 'word2vec_gensim':
        model = KeyedVectors.load("../model/country.model")
...
def list_country(namemethod = 'word2vec_gensim'):
    list_vector_country = getvector(namemethod)
    return sorted(list_vector_country.keys())


def distance_country_euclidean(word1, word2, namemethod = 'word2vec_gensim'):
    list_vector_country = getvector(namemethod)
    print namemethod
